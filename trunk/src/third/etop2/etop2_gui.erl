...
-import(etop2, [loadinfo/1, meminfo/2, getopt/2]).
...
get_event(Info, Win, Grid, GL1, SysInfo, Config) ->
	...
	{gs, accum, _, _, _} ->
	    ...
	    etop2:config(accumulate,not Old),
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);	
	{gs,intv,_,_,_} ->
	    case pop(Win,interval) of
		{ok,Intv} -> etop2:config(interval,list_to_integer(Intv));
		{error,cancel} -> ok
	    end,
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);
	{gs,lines,_,_,_} ->
	    case pop(Win,lines) of
		{ok,Lines} -> etop2:config(lines,list_to_integer(Lines));
		{error,cancel} -> ok
	    end,
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);
	{gs,Sort,_,_,_} when Sort=:=runtime; 
			     ...
			     Sort=:=reductions; 
			     Sort=:=msg_q ->
	    etop2:config(sort,Sort),
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);	
	{gs,GL1,click,_,[Col,1,_]} -> 
	    case Col of 
		?TimeCol -> etop2:config(sort, runtime); 
		?MemCol -> etop2:config(sort, memory); 
		?RedsCol -> etop2:config(sort, reductions); 
		?MsgQCol -> etop2:config(sort, msg_q);
		_other -> ignore
	    end,
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);	
	...
	{dump,Fd} -> 
	    etop2_txt:do_update(Fd,Info,Config),
	    get_event(Info, Win, Grid, GL1, SysInfo, Config);
	Msg ->
