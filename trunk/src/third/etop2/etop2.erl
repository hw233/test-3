...
init_data_handler(Config) ->
    ...
    Reader = 
	if Config#opts.tracing == on -> etop2_tr:reader(Config);
	   true -> undefined
    end,
...
stop(Opts) ->
    ...
    if Opts#opts.tracing == on -> etop2_tr:stop_tracer(Opts);
       true -> ok
    end,
...
handle_args([{interval, [Time]}| R], Config) when is_list(Time)->
    ...
    handle_args(R, NewC);
handle_args([{repc, Repc}| R], Config) when is_integer(Repc)->
    Repc2 = 
    case Repc < 0 of
        true ->
            0;
...
handle_args([{output, [Output]}| R], Config) when is_list(Output) ->
    ...
    handle_args(R, NewC);
handle_args([{outfile, Outfile}| R], Config) when is_list(Outfile) ->
    NewC = Config#opts{outfile=Outfile},
    handle_args(R, NewC);
...
output(graphical) -> etop2_gui;
output(text) -> etop2_txt.
