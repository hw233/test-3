...
execute(PoolId, Sql) ->
	case mysql:fetch(PoolId, Sql) of
        ...
        {error, Info} -> mysql_error(Sql, Info)
    end.
...
%% 处理mysql操作出错（目前是直接抛出异常）
mysql_error(Sql, {_, _, _, _, _, Reason, ErrCode, ErrSqlState}) ->
	?TRACE("[db_error] Sql:~s, ErrCode:~p, ErrState:~s, Reason:~s~n", [Sql, ErrCode, ErrSqlState, Reason]),
...
mysql_error(Sql, Info) ->
	?ERROR_MSG("[db_error] Sql:~s, Info:~p", [Sql, Info]),
	erlang:error({db_error, [Sql, Info]}).
...
make_insert_or_update_sql(Table_name, FieldList, ValueList) ->
    ...
    Fields = make_conn_sql_key(FieldList,[]),
    Values = make_conn_sql_value(ValueList,[]),

    % ?DEBUG_MSG("Fields=~p,Values=~p",[Fields,Values]),

    lists:concat(["insert into `", Table_name,"`", Fields," Values",Values," on duplicate key update ", L]).

% 拼接
make_conn_sql_key([ H | T ],[]) ->
	L = ["(`",tool:to_list(H),"`"],
	make_conn_sql_key(T,L);

make_conn_sql_key([ H | T ],L) ->
	L1 = L ++ [",`",tool:to_list(H),"`"],
	make_conn_sql_key(T,L1);
...
% 拼接
make_conn_sql_value([ H | T ],[]) ->
	L = ["('",tool:to_list(H),"'"],
	make_conn_sql_value(T,L);

make_conn_sql_value([ H | T ],L) ->
	L1 = L ++ [",'",tool:to_list(H),"'"],
	make_conn_sql_value(T,L1);

make_conn_sql_value([],L) ->
	L1 = L ++ [")"],
	L1.
...
%%                         [{status, 0}, {online_flag,1}, {hp,50}, {mp,30}]).
%%	LvTrain=<<"[1,2]">> , Liangong= <<"[1200100000000658,1200100000000657,1200100000000656,1200100000000655,\n 1200100000000654,1200100000000653,1200100000000652,1200100000000651,\n 1200100000000650,1200100000000649,1200100000000648,1200100000000647,\n 1200100000000646,1200100000000645,1200100000000644,1200100000000643,\n 1200100000000642]">>
 	{Vsql, _Count1} =
		lists:mapfoldl(
	  		fun(Field_value, Sum) ->	
				Expr = case Field_value of
