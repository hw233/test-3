...
-define(LOG_FILE_INTERVAL, 3600000). % 1小时新开1个log文件

start_link(Log_file, Log_level) ->
    case gen_event:start_link({global, ?LOGMODULE}) of   
        Ret = {ok, _Pid} ->
            gen_event:add_handler({global,?LOGMODULE}, ?MODULE, [Log_file, Log_level]),
            Ret;   
        Other ->   
            Other   
    end.
...
log_level() ->
	gen_event:call({global,?LOGMODULE}, ?MODULE, log_level). 
...
gen_full_log_filename(File, {{Y,Mo,D},{H,Mi,S}}) ->
	io_lib:format("~s ~w.~.2.0w.~.2.0w ~.2.0w-~.2.0w-~.2.0w", [File, Y, Mo, D, H, Mi, S]).
