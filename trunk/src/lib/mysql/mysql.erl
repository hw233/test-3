...
-export([start_link/5,
	 ...
	 dbg_let_mysql_conn_crash/0,  % just for debug and test mysql reconnect -- add by huangjf
...
start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding,
    ...
    gen_server:StartFunc(
      {local, ?SERVER}, ?MODULE,   % ?SERVER: mysql_dispatcher
...
%% 	io:format("+++ connect:PoolId=~p,Database=~s~n", [PoolId,Database]),
...
new_conn(PoolId, ConnPid, Reconnect, Host, Port, User, Password, Database,
	 Encoding) ->
%% 	io:format("--- PoolId=~p,ConnPid=~p,Database=~s~n",[PoolId,ConnPid,Database]),
    case Reconnect of
	true ->
	    #conn{pool_id = PoolId,
		  pid = ConnPid,
...
fetch(PoolId, Query, Timeout) ->
    case get(?STATE_VAR) of
	undefined ->  % 目前实际运行时是执行此分支
		% ?DEBUG_MSG("mysql, fetch(), get(?STATE_VAR) ret undefined", []),
	    call_server({fetch, PoolId, Query}, Timeout);
	State ->
		% ?DEBUG_MSG("!!!!!mysql, fetch(), get(?STATE_VAR): ~p", [State]),
	    mysql_conn:fetch_local(State, Query)
    end.


%% just for debug and test mysql reconnect -- add by huangjf
dbg_let_mysql_conn_crash() ->
	PoolId = sim_mysql_conn,
	DummyQuery = '__just_for_debug_let_it_crash__',
	fetch(PoolId, DummyQuery).
...
pools() ->
	Ret = gen_server:call(?SERVER, get_pools),
	{ok, Pools} = Ret,
	?DEBUG_MSG("pools:~p, size:~p", [Pools, gb_trees:size(Pools)]),
	Ret.
...
%% 	io:format("*** init:PoolId=~p,Database=~s~n", [PoolId,Database]),
...
create_ets() ->
	ets:new(?ETS_UNHANDLED_SQL_QUERIES_MSG, [named_table, public, set]).
...
handle_call(get_pools, _From, State) ->
    {reply, {ok, State#state.conn_pools}, State}.
...
handle_info({'DOWN', _MonitorRef, process, Pid, Info}, State) ->
	?DEBUG_MSG("mysql recv DOWN msg, ConnPid:~p, Info:~p", [Pid, Info]),
    LogFun = State#state.log_fun,
...
call_server(Msg, Timeout) ->
    if Timeout == undefined ->
	    gen_server:call(?SERVER, Msg, ?DEFAULT_CALL_GENSRV_TIMEOUT);
       true ->
...
add_conn(Conn, State) ->
    ...
    State#state{conn_pools =
		gb_trees:enter(PoolId, NewPool,   % map: PoolId => ConnPool
			       ConnPools),
		pids_pools = gb_trees:enter(Pid, PoolId,  % map: ConnPid => PoolId
...
-define(MAX_RECONN_TRY_TIMES, 240).
-define(KEEP_ON_TRY_RECONN_INTV, 1000).
...
reconnect_loop(Conn, LogFun, N) ->
	...
	{ok, NewConnPid} ->
		?CRITICAL_MSG("reconnect_loop() succeed, NewConnPid:~p", [NewConnPid]),
	    ?Log2(LogFun, debug,
		"managed to reconnect to ~p:~s:~p "
		"(connection pid ~p)", [PoolId, Host, Port, NewConnPid]),

		% 处理连接断开时未完成的sql操作
		OldConnPid = Conn#conn.pid,
		deal_unhandled_sql_queries(OldConnPid, NewConnPid),

	    ok;
	{error, Reason} ->
		?ERROR_MSG("reconnect_loop() failed, Reason:~p", [Reason]),
        ...
        case NewN >= ?MAX_RECONN_TRY_TIMES of
        	true ->
        		?CRITICAL_MSG("reconnect_loop() try max times!!", []),
      			done;
      		false ->
      			%% sleep between every unsuccessful attempt
			    timer:sleep(?KEEP_ON_TRY_RECONN_INTV),  % old: timer:sleep(5000)
...
deal_unhandled_sql_queries(OldConnPid, NewConnPid) ->
	?ASSERT(is_pid(OldConnPid), OldConnPid),
	?ASSERT(is_pid(NewConnPid), NewConnPid),
	case mysql_conn:get_unhandled_sql_queries_msg(OldConnPid) of
		null ->
			?DEBUG_MSG("do_deal_unhandled_sql_queries(), Unnecessary! OldConnPid:~p, NewConnPid:~p", [OldConnPid, NewConnPid]),
			skip;
		SqlQueriesMsgList ->
			spawn(fun() -> do_deal_unhandled_sql_queries(SqlQueriesMsgList, NewConnPid) end),
			mysql_conn:discard_unhandled_sql_queries_msg(OldConnPid),  % 处理后则丢弃
			?ASSERT(mysql_conn:get_unhandled_sql_queries_msg(OldConnPid) == null)
	end.
	

do_deal_unhandled_sql_queries([], _NewConnPid) ->
	?DEBUG_MSG("do_deal_unhandled_sql_queries() done! NewConnPid:~p", [_NewConnPid]),
	done;
do_deal_unhandled_sql_queries([SqlQueriesMsg | T], NewConnPid) ->
	case SqlQueriesMsg of
		{fetch, Queries, From} ->
			?DEBUG_MSG("do_deal_unhandled_sql_queries(), NewConnPid:~p, SqlQueriesMsg:~p", [NewConnPid, SqlQueriesMsg]),
			mysql_conn:fetch(NewConnPid, Queries, From);
		_ ->
			skip
	end,
	do_deal_unhandled_sql_queries(T, NewConnPid).
