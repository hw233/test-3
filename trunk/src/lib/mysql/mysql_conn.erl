...
%%--------------------------------------------------------------------
-export([do_recv/3,
		get_unhandled_sql_queries_msg/1,
		discard_unhandled_sql_queries_msg/1
	]).
...
do_recv(LogFun, RecvPid, SeqNum)  when is_function(LogFun);
				       LogFun == undefined,
				       SeqNum == undefined ->
	% ?DEBUG_MSG("do_recv(), exec branch 1", []),
    receive
...
do_recv(LogFun, RecvPid, SeqNum) when is_function(LogFun);
				      LogFun == undefined,
				      is_integer(SeqNum) ->
	% ?DEBUG_MSG("do_recv(), exec branch 2", []),
...
-define(KN_IS_CONN_CLOSED, kn_is_conn_closed).

mark_conn_closed(Reason) ->
	erlang:put(?KN_IS_CONN_CLOSED, {true, Reason}).

is_conn_closed() ->
	case erlang:get(?KN_IS_CONN_CLOSED) of
...
send_msg(Pid, Msg, From, Timeout) ->
    ...
    case From of
	Self ->
		% ?DEBUG_MSG("send_msg(), NOT using mysql_dispatcher", []),
	...
	_ ->
		% ?DEBUG_MSG("send_msg(), using mysql_dispatcher", []),
	...
	{ok, RecvPid, Sock} ->
		...
		{ok, Version} ->
			...
			{_ResultType, _MySQLRes} ->
				...
				% ?DEBUG_MSG("init(), mysql_version:~p", [Version]),
			    loop(State)
		    end;
		{error, _Reason} ->
...
%%--------------------------------------------------------------------
loop(State) ->
	case is_conn_closed() of
		{true, Reason} ->
			handle_conn_closed(Reason),
			error;
		false ->
		    ...
		    % just for debug and self test -- huangjf
		    {fetch, ['__just_for_debug_let_it_crash__'], _From} ->
		    	?DEBUG_MSG("loop(), __just_for_debug_let_it_crash__", []),
		    	handle_conn_closed('__just_for_debug_let_it_crash__'),
			    error;
		    ...
		    Unknown ->
			    ...
			    handle_conn_closed({unknown, Unknown}),
			    error
		    end
	end.
...
%% 处理连接被关闭
handle_conn_closed(Reason) ->
	collect_unhandled_sql_queries_msg(self()),
	?CRITICAL_MSG("mysql conn closed!! Reason:~p", [Reason]).


get_unhandled_sql_queries_msg(ConnPid) ->
	case ets:lookup(?ETS_UNHANDLED_SQL_QUERIES_MSG, ConnPid) of
		[] ->
			null;
		[{ConnPid, SqlQueriesMsgList}] ->
			SqlQueriesMsgList
	end.

discard_unhandled_sql_queries_msg(ConnPid) ->
	ets:delete(?ETS_UNHANDLED_SQL_QUERIES_MSG, ConnPid).

collect_unhandled_sql_queries_msg(ConnPid) ->
	{messages, MsgList} = erlang:process_info(ConnPid, messages),
	SqlQueriesMsgList = [X || X <- MsgList, is_sql_queries_msg(X)],
	?DEBUG_MSG("collect_unhandled_sql_queries_msg(), ConnPid:~p, SqlQueriesMsgList:~p", [ConnPid, SqlQueriesMsgList]),
	case SqlQueriesMsgList == [] of
		true ->
			skip;
		false ->
			ets:insert(?ETS_UNHANDLED_SQL_QUERIES_MSG, {ConnPid, SqlQueriesMsgList})
	end.

is_sql_queries_msg(Msg) ->
	case Msg of
		{fetch, _Queries, _From} ->
			true;
		_ ->  % 目前，其他形式的统一认为不是sql操作 -- huangjf
			false
	end.
...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
    ...
    case do_send(Sock, Packet, 0, LogFun) of  % SeqNum统一从0开始，why？？
	ok ->
...
get_lcb(<<253:8, Value:24/little, Rest/binary>>) ->
    {Value, Rest};
get_lcb(<<254:8, Value:64/little, Rest/binary>>) ->  % Value对应64位是正确的！
    {Value, Rest};
