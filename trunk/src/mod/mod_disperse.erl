...
rpc_server_add(Id, Node, Ip, Port) ->
	case whereis(?MODULE) of
		undefined ->
			?TRACE("===== [mod_disperse] ~p is not disperse node =====~n", [node()]); % world节点不是战区节点，故没有mod_disperse进程
		Pid ->
    		Pid ! {rpc_server_add, Id, Node, Ip, Port}
	end.
...
handle_info({nodeup, Node}, State) ->
    ?TRACE("[mod_disperse] recv nodeup info! Node:~p~n", [Node]),
    try
...
handle_info({nodedown, Node}, State) ->
    ?TRACE("[mod_disperse] recv nodedown info! Node:~p~n", [Node]),
    %% 检查是否战区节点，并做相应处理
...
add_server([Ip, Port, Sid, Node]) ->
	db:replace(server, [{id,Sid}, {ip,Ip}, {port,Port}, {node,Node}]).

%%退出服务器集群
del_server(Sid) ->
	db:delete(server, [{id,Sid}]).

%%获取并通知所有线路信息
get_and_call_server(State) ->
    case db:select_all(server, "*", []) of
        [] ->
            [];
        Server ->
