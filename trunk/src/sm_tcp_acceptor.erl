...
set_sockopt(LSock, Sock) ->
    ...
    case prim_inet:getopts(LSock, [packet, active, nodelay, keepalive, delay_send, priority, tos]) of
        {ok, Opts} ->
...
%% 开启客户端服务
start_client(Sock) ->
    ?TRACE("start_client, ip: ~p!!!!!!!!!!~n", [misc:get_ip(Sock)]),
    {ok, Child} = supervisor:start_child(sm_tcp_client_sup, []),
    Child ! {go, Sock}.