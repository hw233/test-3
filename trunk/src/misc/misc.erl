...
% %% desc: 创建玩家的任务ets表编号
% create_task_log_ets_name(_Num) ->
%     % case lists:member(Num, lists:seq(1, ?MAX_GOODS_ETS_NUM)) of
%     %     true ->  tool:to_atom( lists:concat([ets_role_task_log_, Num]) );
%     %     false -> none
%     % end.
%     ets_role_task_log.
...
%% @doc 检查IP合法性
%% @return : boolean()
check_ip_validity(Ip) when is_list(Ip) ->
    case ets:lookup(?ETS_SECURITY_IP, Ip) of
        [{Ip}] -> true;
        _ -> false
    end;
check_ip_validity(_) -> false.


check_ip_validity_config(Ip) ->
	case config:check_admin_security_switch() of
        true -> check_ip_validity(Ip);
        false -> true
    end.
...
%% @doc 加载IP白名单
load_security_ips() ->
    case get_ip_config_info() of
        null -> throw({sm_admin, load_security_ips});
        IpList -> 
            lists:foreach(
                fun(Ip) -> ets:insert(?ETS_SECURITY_IP, {Ip}) end, 
                IpList)
    end.
...
update_security_ips() ->
    case get_ip_config_info() of
        null -> {error, load_security_ips_file};
        IpList -> 
            lists:foreach(
                fun(Ip) -> ets:insert(?ETS_SECURITY_IP, {Ip}) end, 
                IpList)
...
-define(SECURITY_FILE_NAME, "admin_security_ip.config").
%% 取得配置文件信息
%% @return : null | Data::list()
get_ip_config_info() -> 
    case file:get_cwd() of
        {ok, Dir} ->
            File = Dir ++ "/" ++ ?SECURITY_FILE_NAME,
            case file:open(File, [read]) of
                {ok, Stream} ->
                    Result = case catch io:read(Stream, '') of
                        {ok, DataList} when is_list(DataList) ->
                            case lists:keyfind(security_ip, 1, DataList) of
                            	{security_ip, Data} when is_list(Data) -> Data;
                            	_T ->
                            		?ERROR_MSG("get_ip_config_info keyfind error = ~p, DataList = ~p~n", [_T, DataList]),
                            		null
                            end;
                        Err ->
                            ?ERROR_MSG("get_ip_config_info IO read file error = ~p~n", [Err]),
                            null
                    end,
                    file:close(Stream),
                    Result;
                Err ->
                    ?ERROR_MSG("get_ip_config_info open file = ~p error = ~p~n", [File, Err]),
                    null
            end;
        {error, Reason} ->
            ?ERROR_MSG("get_ip_config_info get_cwd error = ~p~n", [Reason]),
            null
    end.

%% @doc 反编译
abstract(Module) ->
	io:fwrite("~s~n", [recon:source(Module)]).

get_pid_info() ->
	case player:this_is_player_proc() of
		true ->
			PlayerId = get(?PDKN_PLAYER_ID),
			io_lib:format("[playerid:~p]",[PlayerId]);
		false ->
			case erlang:process_info(self(), registered_name) of
				{_, RegistedName} ->
					io_lib:format("[regname:~p]",[RegistedName]);
				_ ->
					case erlang:process_info(self(), initial_call) of
						{_, {M,F,ArgNum}} ->
							io_lib:format("[mfa:~p:~p/~p]",[M,F,ArgNum]);
						_ ->
							io_lib:format("[none]",[])
					end
			end
