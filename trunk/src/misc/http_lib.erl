...
-export([uri_to_path/1, encode/1, list_to_absoluteURI/1, etag/1,
	 ...
	 url_encode/1, url_decode/1, chunk/1, response/1, response/2,
	 accept/1, recv/2, recv/3, send/2, setopts/2, close/1, peername/1,
	 dir/1, is_persistent/1, is_idempotent/1, is_modified/2, url_encode_cn/1,
	 reason_phrase/1]).
...
url_encode_cn([],Acc) ->
    Acc;
url_encode_cn([{Key,Value}|R],"") ->
    url_encode_cn(R, escape_uri_cn(Key) ++ "=" ++ escape_uri_cn(Value));
url_encode_cn([{Key,Value}|R],Acc) ->
    url_encode_cn(R, Acc ++ "&" ++ escape_uri_cn(Key) ++ "=" ++ escape_uri_cn(Value)).
...
%%中文urlencode
escape_uri_cn([C | Cs]) when C >= $a, C =< $z ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C | Cs]) when C >= $A, C =< $Z ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C | Cs]) when C >= $0, C =< $9 ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C = $. | Cs]) ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C = $- | Cs]) ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C = $_ | Cs]) ->
    [C | escape_uri_cn(Cs)];
escape_uri_cn([C | Cs]) ->
    escape_byte(C) ++ escape_uri_cn(Cs);
escape_uri_cn([]) ->
    [].
