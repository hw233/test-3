...
%%从列表里随机取N个元素（TODO: 目前实现的算法效率不高，考虑是否有更优化的做法）
shuffle(List, N) -> ?ASSERT(length(List) >= N), shuffle__(List, length(List), N, []).

shuffle__(_List, _ListLen, N, Acc) when N == 0 -> Acc;
shuffle__(List, ListLen, N, Acc) ->
    {Leading, [H | T]} = lists:split(util:rand(1, ListLen) - 1, List),
    shuffle__(Leading ++ T, ListLen - 1, N - 1, [H | Acc]).

%% 用于协议接收从客户端过来的整数数组数据
%% @return: [] | 整数列表
read_int32_array(Bin) ->
    case Bin of
        <<Len:16, Bin1/binary>> ->
            read_int32_array__(Len,Bin1,[]);
        _R1 ->
            []
    end.

read_int32_array__(0, _Rest, Reply) ->
	lists:reverse(Reply);
read_int32_array__(Len, Bin, Reply) ->
	<<Num:32, Bin1/binary>> = Bin,
	read_int32_array__(Len - 1, Bin1, [Num | Reply]).