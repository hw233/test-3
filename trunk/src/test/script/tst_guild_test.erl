...
%    case gen_tcp:connect(?HOST, ?PORT, [binary, {packet, 0}], 100000) of
    case gen_tcp:connect("localhost", 6666, [binary, {packet, 0}], 100000) of
        {ok, Socket} ->
            ...
            sleep(10),
            % 登录消息10000
            handle(login, {N, integer_to_list(1111)}, Socket),
            sleep(10),
            % 创建角色消息10003
...
%            sleep(10),
            %% 踢出帮派
            handle(kickout_guild, {2, 4}, Socket),
            sleep(10),
            %% 帮派列表
...
%%踢出帮派
handle(kickout_guild, {GuildId, PlayerId}, Socket) ->
    io:format("client send: kickout_guild Cmd=[40008], GuildId = [~p] ,PlayerId=[~p] ~n", [GuildId, PlayerId]),
    MsgLen = byte_size(<<1:16,40008:16,GuildId:32, PlayerId:32>>),
    gen_tcp:send(Socket, <<MsgLen:16,40008:16,GuildId:32, PlayerId:32>>),
    ok;%rec(Socket);
