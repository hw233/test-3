...
create(Socket) -> 
	L1 = byte_size(<<"逍遥">>),
    L = byte_size( <<1:16,10003:16,1:8,1:8,1:8,L1:16,"逍遥">>),
    gen_tcp:send(Socket, <<L:16,10003:16,1:8,1:8,1:8,L1:16,"逍遥">>),
...
%%用户信息
load_player(Socket) ->
	io:format("send:13001~n"),
    gen_tcp:send(Socket, <<6:16,13001:16,2:16>>),
    rec(Socket).

%%获取对话
get_npc(Socket) ->
    io:format("send:32000~n"),
    gen_tcp:send(Socket, <<8:16,32000:16,10101:32>>),
    rec(Socket).
